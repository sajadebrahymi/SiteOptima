...
function total_weighted_time = calculate_fitness(crane_positions, problem)
    ...
    num_cranes = cs.num_cranes; % This will be 6 in the new scenario
    R_max = cs.max_radius;
    ...
    for k_constr = 1:num_cranes % Corrected loop variable name
    ...
    if num_cranes > 1
        for k1 = 1:num_cranes
            for k2 = (k1+1):num_cranes
                ...
                if dist_sq < min_dist_cranes_sq && dist_sq > 1e-9 % Avoid issues if cranes are at the exact same spot initially
                    total_weighted_time = total_weighted_time + large_penalty; 
                end
            end
        end
    end
    % If already heavily penalized by inter-crane distance, consider returning early
    ...
    all_demands_covered_this_solution = true; % Renamed for clarity
    for j = 1:num_D 
        Dj_coord = D(j, 1:2);
        Dj_height = D(j, 3);
        Qj_tonnes = D(j, 4);

        min_time_for_Dj_this_solution = inf; % Renamed for clarity

        num_cycles_for_Dj = ceil(Qj_tonnes / cap_per_cycle);

        for k_calc = 1:num_cranes % Corrected loop variable name
            ...
            for i_calc = 1:num_S % Corrected loop variable name
    ...
    % Additional penalty if solution is generally bad (e.g. high inter-crane penalty AND uncovered demands)
    % This part is optional and depends on how strictly you want to penalize.
    % if total_weighted_time >= large_penalty && ~all_demands_covered_this_solution
    %    total_weighted_time = total_weighted_time + large_penalty; % Further penalize very bad solutions
    % end
end