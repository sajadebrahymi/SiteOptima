% -------------------------------------------------------------------------
% MAIN SCRIPT for Crane Layout Optimization - SCENARIO: RHO DECAY RATE CHANGE
% Based on "Base Scenario" (v2 with explicit decay params)
% Changes:
%   - Number of cranes: 8
%   - rho_decay_rate (Search Radius Reduction Rate) changed to 0.90.
%   - Demand points use ORIGINAL weights.
% -------------------------------------------------------------------------
clear;
close all;
clc;

fprintf('Starting Crane Layout Optimization - SCENARIO: RHO DECAY RATE CHANGE\n');
...
% Demand Points (D_j) [x, y, z_d, Q_j_tonnes] - ORIGINAL Weights from Base Scenario
...
% --- DEA Algorithm Parameters (from Base Scenario v2, with rho_decay_rate changed) ---
dea_params.Npop = 50; 
dea_params.Tmax = 200; 
dea_params.num_pulses_per_dolphin = 10;
dea_params.R_init = 0.1 * site_dims.width; 
% ******** KEY CHANGE HERE ********
dea_params.rho_decay_rate = 0.90; % <--- Changed from 0.99 to 0.90
% ***********************************
dea_params.delta_decay_rate = 0.995; % Kept as in base scenario
...
fprintf('  Scenario: Rho Decay Rate Change\n');
...
% --- Results ---
fprintf('Optimization Finished for Rho Decay Rate Change Scenario.\n');
