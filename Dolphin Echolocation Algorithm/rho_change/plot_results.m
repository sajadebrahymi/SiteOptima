...
function plot_results(best_positions, problem, fitness_history)
    ...
    if ~isempty(best_positions)
        num_cranes_to_plot = problem.crane_specs.num_cranes; % Will be 6 for the new scenario
        crane_colors = lines(num_cranes_to_plot); 
        for k_plot = 1:num_cranes_to_plot % Corrected loop variable
    ...
    axis equal;
    axis([0 problem.site_dims.width 0 problem.site_dims.length]); % Ensure axis limits cover the site
    ...
    if problem.crane_specs.num_cranes > 0 % Only show legend if there are cranes/points
    ...
    subplot(1,2,2);
    if ~isempty(fitness_history) && any(~isinf(fitness_history)) % Plot if history is valid
        ...
        elseif ~isempty(ylim_min)
             ylim([ylim_min - 0.1*abs(ylim_min), ylim_min + 1]); % Handle case where fitness is constant
        end
    else
