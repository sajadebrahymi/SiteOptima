% -------------------------------------------------------------------------
% MAIN SCRIPT for Crane Layout Optimization - SCENARIO: SITE WIDTH REDUCTION
% Based on "Base Scenario" (v2 with explicit decay params)
% Changes:
%   - Number of cranes: 8
%   - Site Y-dimension (length/عرض) reduced from 75m to 60m.
%   - Demand points use ORIGINAL weights and coordinates.
% -------------------------------------------------------------------------
clear;
close all;
clc;

fprintf('Starting Crane Layout Optimization - SCENARIO: SITE WIDTH REDUCTION\n');

% --- Problem Definition ---
% Site Dimensions
% ******** KEY CHANGE HERE ********
site_dims.width = 250; % meters (X-axis, remains unchanged)
site_dims.length = 60;  % meters (Y-axis, "عرض" reduced from 75m to 60m)
% ***********************************
fprintf('  Site Dimensions: %.1fm x %.1fm\n', site_dims.width, site_dims.length);

% Supply Points (S_i) [x, y, z_s] - Original
supply_points = [
    40,   20.5, 0;
    235,  20.5, 0;
    137.5, 29.25,0
...
% Demand Points (D_j) [x, y, z_d, Q_j_tonnes] - ORIGINAL Weights and Coordinates
...
% --- DEA Algorithm Parameters (from Base Scenario v2) ---
dea_params.Npop = 50; 
dea_params.Tmax = 200; 
dea_params.num_pulses_per_dolphin = 10;
% R_init is based on the larger dimension (site_dims.width) as before
dea_params.R_init = 0.1 * site_dims.width; 
dea_params.rho_decay_rate = 0.99; % As in base scenario
dea_params.delta_decay_rate = 0.995; % As in base scenario

% --- Pack all problem information into a structure ---
problem.site_dims = site_dims; % Contains updated dimensions
problem.supply_points = supply_points;
problem.num_supply_points = num_supply_points;
problem.demand_points = demand_points_original; % Use ORIGINAL demand points
...
fprintf('  Scenario: Site Width Reduction\n');
...
% --- Results ---
fprintf('Optimization Finished for Site Width Reduction Scenario.\n');
