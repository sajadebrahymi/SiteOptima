% -------------------------------------------------------------------------
% Dolphin Echolocation Algorithm (DEA) for Crane Layout Optimization
% Version 2: Incorporating R_init, rho_decay_rate, delta_decay_rate
% (This file is unchanged from the "Base Scenario")
% -------------------------------------------------------------------------
function [best_overall_positions, best_overall_fitness, fitness_history] = ...
    ...
    M = dea_params.num_pulses_per_dolphin; % Pulses per dolphin
    
    R_init = dea_params.R_init;
    rho_decay_rate = dea_params.rho_decay_rate;
    delta_decay_rate = dea_params.delta_decay_rate;
    initial_global_best_influence = 1.0; % Initial influence factor for global best component

    num_cranes = problem.crane_specs.num_cranes; % This will be 6 in the new scenario
    ...
    for i = 1:N
        ...
        for k_init = 1:num_cranes % Corrected loop variable name for clarity
            valid_pos = false;
            attempts = 0;
            while ~valid_pos && attempts < 100 
                ...
                if k_init > 1
                    ...
                    if min_dist_to_others >= problem.crane_specs.min_dist_between_cranes * 0.5 
                        valid_pos = true;
                    end
                else
                    valid_pos = true;
    ...
    if ~isempty(best_overall_fitness) % Ensure best_overall_fitness is not empty
        fitness_history(1) = best_overall_fitness;
    else % Handle cases where initialization might yield no valid solutions (highly unlikely with current setup)
        fitness_history(1) = inf;
        % Potentially exit or re-initialize if all initial solutions are inf
        if isinf(best_overall_fitness)
             warning('Initial population yielded no valid solutions (all fitness values are Inf). Check constraints or initialization.');
        end
    end
        ...
        for i = 1:N 
            ...
            for p = 1:M
                ...
                elseif strategy_rand < 0.9 
                    ...
                    indices_to_perturb = randperm(num_cranes, num_cranes_to_perturb); % Select unique cranes to perturb
            ...
            [best_pulse_fitness, best_pulse_idx] = min([candidate_pulses.fitness]);
             if ~isempty(best_pulse_idx) % Check if any valid pulse was found
