% -------------------------------------------------------------------------
% MAIN SCRIPT for Crane Layout Optimization using PARTICLE SWARM OPTIMIZATION (PSO)
...
fprintf('Starting Crane Layout Optimization using PARTICLE SWARM OPTIMIZATION (PSO)...\n');
...
pso_params.w_final = 0.4;   % Final inertia weight (for linear damping)
pso_params.c1 = 2.0;        % Cognitive coefficient (pbest influence)
pso_params.c2 = 2.0;        % Social coefficient (gbest influence)
% Max velocity as a ratio of the search space dimension
pso_params.Vmax_ratio_x = 0.2; % For x-coordinates, e.g., 0.2 * site_dims.width
pso_params.Vmax_ratio_y = 0.2; % For y-coordinates, e.g., 0.2 * site_dims.length
...
fprintf('  NumParticles = %d, MaxIter = %d, w = %.1f->%.1f, c1 = %.1f, c2 = %.1f\n', ...
    pso_params.num_particles, pso_params.MaxIter, pso_params.w_initial, pso_params.w_final, ...
    pso_params.c1, pso_params.c2);

[best_crane_positions, best_fitness_value, fitness_history] = ...
    pso_optimizer(problem, pso_params); 

% --- Results ---
fprintf('Optimization Finished with Particle Swarm Optimization.\n');
